var ws = {
    register: function () {
        ...
        if (socket.readyState == WebSocket.OPEN) {
            ...
            $("#alertContent").html('<p>Websocket连接没有开启！</p>')
            $("#alert").click();
    ...
    registerReceive: function (data) {
        ...
        for (var i = 0; i < groupList.length; i++) {
            sentMessageMap.set("g" + groupList[i].groupId, new Array());
            let members = groupList[i].members;
            for (var j = 0; j < members.length; j++) {
                userMap.set(members[j].userId, members[j]);
            }
            groupListHTML +=
                '<li>' +
        ...
        for (var i = 0; i < friendList.length; i++) {
            userMap.set(friendList[i].userId, friendList[i]);
            sentMessageMap.set("u" + friendList[i].userId, new Array());
            friendListHTML +=
                '<li>' +
                '<div class="liLeft"><img src="' + friendList[i].pic + '"></div>' +
                '<div class="liRight">' +
    ...
    singleReceive: function (data) {
        ...
        var fromAvatarUrl = userMap.get(fromUserId).pic;
        var $receiveLi;
...
function friendLiClickEvent() {
    ...
    var messageArray;
    if (toUserId.length != 0) {
        messageArray = sentMessageMap.get('u' + toUserId);
        $('#toUserId').val(toUserId);
    } else {
        messageArray = sentMessageMap.get('g' + toGroupId);
...
var processMsgBox = {
    sendMsg: function (msg, toUserId, toGroupId) {
        ...
        if (toUserId.length != 0) {
            sentMessageMap.get('u' + toUserId).push($('.newsList li').last().prop("outerHTML"));
        } else {
            sentMessageMap.get('g' + toGroupId).push($('.newsList li').last().prop("outerHTML"));
        }
    ...
    receiveSingleMsg: function (msg, fromUserId) {
        ...
        sentMessageMap.get('u' + fromUserId).push($('.newsList-temp li').last().prop("outerHTML"));
        $('.newsList-temp').empty();
    ...
    receiveGroupMsg: function (msg, toGroupId) {
        ...
        // 4. 把 调整后的消息html标签字符串 添加到已发送用户消息表，并清空暂存区
        sentMessageMap.get('g' + toGroupId).push($('.newsList-temp li').last().prop("outerHTML"));
        $('.newsList-temp').empty();
